...
class TraderAPI(object):
    ...
    def _login(self, v_code):
        ...
        login_params = dict(
            ...
            f_tdx='',
            j_cpu='',
            j_password=brokerinfo.iloc[0]['passwd']
        )
    ...
    def new_stk_list(self):
        time.sleep(2)
        url = vs.ENTRUST_LIST_URL % (vs.P_TYPE['https'],
                                     vs.DOMAINS['csc'],
                                     vs.PAGES['newstk'],
                                     utils.nowtime_str())
        txtdata = self.s.get(url)
        jsonobj = utils.get_jdata(txtdata)
        df = pd.DataFrame(jsonobj['dataList'], columns=vs.NEWSTK_LIST_COLS)
        return df
    ...
    def _get_position(self):
        time.sleep(3)
        self.s.headers.update(vs.AGENT)
        url = vs.BASE_URL % (vs.P_TYPE['https'],
                                            vs.DOMAINS['csc'],
                                            vs.PAGES['position'])
        txtdata = self.s.get(url)
        jsonobj = utils.get_jdata(txtdata)
        df = pd.DataFrame(jsonobj['data'], columns=vs.POSITION_COLS)
        df['stkqty'] = df['stkqty'].astype(float)
